...
public class TrafficLight: UIView {
  ...
  //These layers hold onto the green/yellow/red states as sublayers
  ...
  public init(canisterCount: Int = 3,
              frame: CGRect = CGRect(x: 0,
                                     y: 0,
                                     width: 160,
                                     height: 420), states: [TrafficLightState]) {
  ...
  private func createCanisterLayers(count: Int) {
    ...
    let paddingPercantage: CGFloat = 0.2
    let yTotalPadding = paddingPercantage * bounds.height
    ...
    //loop from 0 to count to crate canisterShape for required number of canister
    //after each canisterShape add to canisterLayers
    //
    for _ in 0 ..< count {
...
public protocol TrafficLightState: class {
  ...
  //MARK: - Properties
  // time interval to light
  var delay: TimeInterval { get }
  
  //MARK: - Instance Methods
  //each concreate state implement
...
extension SolidTrafficLightState {
  public class func greenLight(color: UIColor =
  ...
  public class func yellowLight(color: UIColor =
    UIColor(red: 0.95, green: 0.91, blue: 0.07, alpha: 1),
                                canisterIndex: Int = 1,
                                delay: TimeInterval = 0.5) -> SolidTrafficLightState {
    return SolidTrafficLightState(canisterIndex: canisterIndex, color: color, delay: delay)
  }
  
  public class func redLight(color: UIColor =
